...
file_root = "/data/gpsro_data_hires/test_2020/raw"
output_dir = "/data/gpsro_data_hires/test_2020/preproc_mask"
...
#matrix_shape = (19, 37)  # 10 degree resolution
# degree = 10.
matrix_shape = (91, 181) #  3 degree resolution
degree = 2.
...
for tag in tqdm(tags):
    ...
    lon = [ ((x + 180) / degree).astype(np.int) for x in lon]
    # load lat
    lat = np.load(os.path.join(file_root,"lat_"+tag+".data"), allow_pickle=True, encoding='latin1')
    lat = [ ((x + 90) / degree).astype(np.int) for x in lat]
    # load data in
    ...
    data_in_mat = [ coo_matrix( (x[0], (x[1], x[2])), shape=matrix_shape ) for x in zip(data_in, lat, lon) ]
    data_out_mat = [ coo_matrix( (x[0], (x[1], x[2])), shape=matrix_shape ) for x in zip(data_out, lat, lon) ]
    mask_mat = [ coo_matrix( (np.ones(len(x[0]), dtype=np.float32), (x[0], x[1])), shape=matrix_shape ) for x in zip(lat, lon) ]
