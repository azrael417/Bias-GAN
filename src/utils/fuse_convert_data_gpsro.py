...
for fname in files:
    #data
    data = np.load(os.path.join(data_path_prefix, "raw", "data_in_"+fname)).astype(np.float32)
    data = np.transpose(data, (2,0,1))
    np.save(os.path.join(outdir, "data_in_"+fname), data)

    #label
    label = np.load(os.path.join(data_path_prefix, "raw", "data_out_"+fname)).astype(np.float32)
