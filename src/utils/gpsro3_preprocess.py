...
for tag in tags:
    ...
    # create dense arrays
    data_in_arr = np.stack([x.astype(np.float32).toarray() for x in data_in_mat], axis=-1)
    data_out_arr = np.stack([x.astype(np.float32).toarray() for x in data_out_mat], axis=-1)
    mask_arr = np.stack([x.toarray() for x in mask_mat], axis=-1)
    ...
    np.save(os.path.join(output_dir, "masks_" + tag + ".npy"), mask_mat)
