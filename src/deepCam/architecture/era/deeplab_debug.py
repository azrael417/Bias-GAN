...
class DeconvUpsampler(nn.Module):
    def __init__(self, n_output):
        ...
        self.deconv2 = nn.Sequential(nn.ConvTranspose2d(256, 256, kernel_size=3, stride=2, padding=1, output_padding=(0,1), bias=False),
                                     nn.BatchNorm2d(256),
                                     nn.ReLU())
	...
	#using 128 intermediate channels because of memory requirements
        self.deconv3 = nn.Sequential(nn.ConvTranspose2d(256, 128, kernel_size=3, stride=2, padding=1, output_padding=(0,1), bias=False),
                                     nn.BatchNorm2d(128),
                                     nn.ReLU())
...
class DeepLabv3_plus(nn.Module):
    def __init__(self, nInputChannels=3, n_output=21, os=16, pretrained=False, _print=True, normalizer=nn.BatchNorm2d):
        ...
        # upsampling
        self.upsample = InterpolationUpsampler(n_output)
