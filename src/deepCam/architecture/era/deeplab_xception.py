...
class SeparableConv2d(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, padding=0, dilation=1, bias=False):
        ...
        #only the last one should have bias bc we do not insert batchnorm in between
...
class SeparableConv2d_same(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, dilation=1, bias=False):
        ...
        #only the last conv should have bias bc we do not insert bn in between
...
class Block(nn.Module):
    def __init__(self, inplanes, planes, reps, stride=1, dilation=1, start_with_relu=True, \
                 grow_first=True, is_last=False, normalizer=nn.BatchNorm2d):
        ...
        if planes != inplanes or stride != 1:
            self.skip = nn.Conv2d(inplanes, planes, 1, stride=stride, bias=bias_flag)
        ...
        if grow_first:
            ...
            rep.append(SeparableConv2d_same(inplanes, planes, 3, stride=1, dilation=dilation, bias=bias_flag))
            rep.append(normalizer(planes))
            filters = planes

        for i in range(reps - 1):
            rep.append(self.relu)
            rep.append(SeparableConv2d_same(filters, filters, 3, stride=1, dilation=dilation, bias=bias_flag))
        ...
        #no bias after a block! we just replace BN with biases and in between blocks there are no BN
        if stride != 1:
            rep.append(SeparableConv2d_same(planes, planes, 3, stride=2, bias=False))

        if stride == 1 and is_last:
            rep.append(SeparableConv2d_same(planes, planes, 3, stride=1, bias=False))
...
class Xception(nn.Module):
    ...
    def __init__(self, inplanes=3, os=16, pretrained=False, normalizer=nn.BatchNorm2d):
        ...
        # Entry flow
        self.conv1 = nn.Conv2d(inplanes, 32, 3, stride=2, padding=1, bias=bias_flag)
        self.bn1 = normalizer(32)
        self.relu = nn.ReLU(inplace=True)

        self.conv2 = nn.Conv2d(32, 64, 3, stride=1, padding=1, bias=bias_flag)
        ...
        self.conv3 = SeparableConv2d_same(1024, 1536, 3, stride=1, dilation=exit_block_rates[1], bias=bias_flag)
        self.bn3 = normalizer(1536)

        self.conv4 = SeparableConv2d_same(1536, 1536, 3, stride=1, dilation=exit_block_rates[1], bias=bias_flag)
        self.bn4 = normalizer(1536)

        self.conv5 = SeparableConv2d_same(1536, 2048, 3, stride=1, dilation=exit_block_rates[1], bias=bias_flag)
...
class ASPP_module(nn.Module):
    def __init__(self, inplanes, planes, rate, normalizer=nn.BatchNorm2d):
        ...
        self.atrous_convolution = nn.Conv2d(inplanes, planes, kernel_size=kernel_size,
                                            stride=1, padding=padding, dilation=rate, bias=bias_flag)
...
class InterpolationUpsampler(nn.Module):
    def __init__(self, n_output, normalizer=nn.BatchNorm2d):
        ...
        self.last_conv = nn.Sequential(nn.Conv2d(304, 256, kernel_size=3, stride=1, padding=1, bias=bias_flag),
                                       normalizer(256),
                                       nn.ReLU(),
                                       nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=bias_flag),
                                       normalizer(256),
                                       nn.ReLU(),
...
class DeconvUpsampler(nn.Module):
    def __init__(self, n_output, normalizer=nn.BatchNorm2d):
        ...
        self.deconv1 = nn.Sequential(nn.ConvTranspose2d(256, 256, kernel_size=3, stride=2, padding=1, output_padding=(0,1), bias=bias_flag),
                                     normalizer(256),
                                     nn.ReLU())

        self.deconv2 = nn.Sequential(nn.ConvTranspose2d(256, 256, kernel_size=3, stride=2, padding=1, output_padding=(0,1), bias=bias_flag),
                                     normalizer(256),
                                     nn.ReLU())

        self.conv1 = nn.Sequential(nn.Conv2d(304, 256, kernel_size=3, stride=1, padding=1, bias=bias_flag),
                                   normalizer(256),
                                   nn.ReLU(),
                                   nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=bias_flag),
                                   normalizer(256),
        ...
        self.deconv3 = nn.Sequential(nn.ConvTranspose2d(256, 128, kernel_size=3, stride=2, padding=1, output_padding=(0,1), bias=bias_flag),
                                     normalizer(128),
                                     nn.ReLU())

        #no bias on the last deconv
        self.last_deconv = nn.Sequential(nn.ConvTranspose2d(128, n_output, kernel_size=3, stride=2, padding=1, output_padding=(0,1), bias=False))
...
class DeepLabv3_plus(nn.Module):
    def __init__(self, nInputChannels=3, n_output=21, os=16, pretrained=False, _print=True, normalizer=nn.BatchNorm2d):
        if _print:
        ...
        # Atrous Conv
        self.xception_features = Xception(nInputChannels, os, pretrained, normalizer=normalizer)
        ...
        self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
                                             nn.Conv2d(2048, 256, 1, stride=1, bias=bias_flag),
                                             normalizer(256),
                                             nn.ReLU())

        self.conv1 = nn.Conv2d(1280, 256, 1, bias=bias_flag)
        self.bn1 = normalizer(256)

        # adopt [1x1, 48] for channel reduction.
        self.conv2 = nn.Conv2d(128, 48, 1, bias=bias_flag)
        self.bn2 = normalizer(48)

        # upsampler
        self.upsample = InterpolationUpsampler(n_output, normalizer)
        #self.upsample = DeconvUpsampler(n_output)
...
if __name__ == "__main__":
    model = DeepLabv3_plus(nInputChannels=3, n_output=21, os=16, pretrained=True, _print=True)
    model.eval()
