...
def main(pargs):
    ...
    validation_set = gpsro.GPSRODataset(validation_dir,
                                        ...
                                        send_device = device)
    validation_loader = DataLoader(validation_set, 1, drop_last=True)
    ...
    with torch.no_grad():
        ...
        for step_val, token_val in enumerate(validation_loader):
            ...
            #get datestamp
            datestamp = filename[0].replace(".npy","")
            ...
            if (step_val % pargs.validation_visualization_frequency == 0):
                sample_idx = 0
        ...
        # we need to increase step_val by
