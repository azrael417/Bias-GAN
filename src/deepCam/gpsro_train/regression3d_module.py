...
class Regression3d(object):
    ...
    def __init__(self, config):
        ...
        n_output_channels = 1
        self.net = dxc.DeepLab3d(n_input = n_input_channels, n_output = n_output_channels, os = 16, 
                                 upsampler_type = self.config["upsampler_type"], pretrained = False, normalizer = normalizer)
        ...
        while True:
            ...
            for token in self.train_loader:
                ...
                with amp.autocast(enabled = self.config["enable_amp"]):
                    ...
                    if self.config["enable_masks"]:
                        loss_dict = {"valid": self.criterion(outputs, label, masks), "hole": self.criterion(outputs, label, 1. - masks)}
                        loss = 0.
                        loss_norm = 0.
                        for key in loss_dict:
                            loss += loss_dict[key] * self.loss_weights[key]
                            loss_norm += self.loss_weights[key]
                    else:
    ...
    def validate(self, step, epoch):
        ...
        with torch.no_grad():
            ...
            for step_val, token_val in enumerate(self.validation_loader):
                ...
                # Compute loss and average across nodes
                if self.config["enable_masks"]:
                    loss_dict = {"valid": self.criterion(outputs_val, label_val, masks_val),
                                 "hole": self.criterion(outputs_val, label_val, 1. - masks_val)}
                    loss_val = 0.
                    loss_norm_val = 0.
                    for key in loss_dict:
                        loss_val += loss_dict[key] * self.loss_weights[key]
                        loss_norm_val += self.loss_weights[key]
                else:
