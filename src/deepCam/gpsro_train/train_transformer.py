...
from architecture.gpsro.TTSR import TTSR as ttsr
...
def main(pargs):
    ...
    # init communicator
    comm = distcomm(mode="openmpi-nccl")
    ...
    net = ttsr.TTSR(in_channels = n_input_channels, out_channels = n_output_channels, num_res_blocks = [16, 16, 8, 4], n_feats = 64, res_scale=1.)
    net.to(device)
    ...
    train_set = gpsro.GPSRODataset(train_dir,
                                   ...
                                   send_device = device)
    train_loader = DataLoader(train_set, 2 * pargs.local_batch_size, drop_last=True)
    ...
    validation_set = gpsro.GPSRODataset(validation_dir,
                                        ...
                                        read_device = torch.device("cpu") if pargs.disable_gds else device,
                                        send_device = device)
    validation_loader = DataLoader(validation_set, 2 * pargs.local_batch_size, drop_last=True)
    ...
    while True:
        ...
        for token in train_loader:
            ...
            # forward pass
            outputs, _, _, _, _ = net(lr = inputs[:pargs.batch_size], lrsr = label[:pargs.batch_size],
                                      ref = inputs[pargs.batch_size:], refsr = label[pargs.batch_size:])
            ...
            if (step % pargs.validation_frequency == 0):
                ...
                with torch.no_grad():
                    ...
                    for step_val, token_val in enumerate(validation_loader):
                        ...
                        # forward pass
                        outputs_val, _, _, _, _ = net(lr = inputs_val[:pargs.batch_size], lrsr = label_val[:pargs.batch_size],
                                                      ref = inputs_val[pargs.batch_size:], refsr = label_val[pargs.batch_size:])
                ...
                val_r2_global = comm.metric_average(val_r2, "val_r2", op_name="sum", device=device)
                val_r2_avg = val_r2_global / count_val_global
