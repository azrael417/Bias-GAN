...
from data import gpsro_graph_dataset as gpsro
from architecture.gpsro import graph_unet as grph
...
class GraphRegression(object):
    ...
    def __init__(self, config):
        ...
        self.net = grph.GraphConvUNet(num_input_channels = n_input_channels,
    ...
    def train(self):
        ...
        while True:
            ...
            for token in self.train_loader:
                ...
                # extract stuff
                lap, data, label, mask, filename = token()
                
                # upload to device
                lap, data, label, mask = map(lambda t: t.to(self.device), (lap, data, label, mask))
                
                # forward pass
                #with amp.autocast(enabled = self.config["enable_amp"]):
                lap_out, data_out = self.net(lap, data)
